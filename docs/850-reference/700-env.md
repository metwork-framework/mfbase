...
`MFBASE_HOME` | alias of `MFMODULE_HOME` if the current loaded metwork module is `mfbase` | `/opt/metwork-mfserv-0.8`
`MFBASE_VERSION` | alias of `MFMODULE_VERSION` if the current loaded metwork module is `mfbase` | `0.9.4`, `integration.ci1036.1949d86`
...
Name | Description | Examples
--- | --- | ---
`MFBASE_CURRENT_PLUGIN_NAME` | the plugin name | `myplugin`
`MFBASE_CURRENT_PLUGIN_DIR` | the plugin directory fullpath | `/home/mfbase/var/plugins/myplugin`
`MFBASE_CURRENT_PLUGIN_LABEL` | the plugin label (see layerapi2 layer labels) | `plugin_myplugin@mfbase`
`MFBASE_CURRENT_PLUGIN_CUSTOM_*` | all variables defined in `[custom]` configuration group of your plugin `config.ini` | `whatever`
