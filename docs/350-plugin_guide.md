...
```
{% raw %}{{MFBASE_HOME}}/bin/cronwrap.sh --lock --log-capture-to your_command.log -- plugin_wrapper {{MFSERV_CURRENT_PLUGIN_NAME}} --
```

It will:

- replace automatically `{{MFBASE_HOME}}`, `{{MFSERV_CURRENT_PLUGIN_NAME}}` and `{{MFSERV_CURRENT_PLUGIN_DIR}}{% raw %}` variables (to avoid hardcoding things)
